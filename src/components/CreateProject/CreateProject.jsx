...
function CreateProject() {
    ...
    const addMember = () => {
        if (username && role) {
            ...
            setUsername(""); // Сброс после добавления
            setRole(""); // Сброс после добавления
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!projectLogo || !projectName || !projectDescription || !projectCategory || !projectStage) {
        ...
        try {
            const response = await fetch('http://localhost:8083/api/v1/main/project', {
                method: 'POST',
                body: formData,
                headers: {
                ...
                <div className={styles.createProjectContainer}>
                    ...
                    <form className={styles.createProjectForm} onSubmit={handleSubmit}>
                        ...
                        <div className={styles.memberInputContainer}>
                            ...
                            <button onClick={addMember} className={styles.addMemberButton}>+</button>
                        </div>
                        <div className={styles.membersList}>
                            {members.map((member, index) => (
                                <div key={index} className={styles.memberItem}>
                                    {member.username} ({member.role})
                                    <button onClick={() => removeMember(index)} className={styles.removeMemberButton}>Удалить</button>
                                </div>
                            ))}
                        </div>
